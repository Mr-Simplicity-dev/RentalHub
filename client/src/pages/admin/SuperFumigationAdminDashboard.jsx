...
const SuperFumigationAdminDashboard = () => {
  ...
  const loadData = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to load national fumigation data');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    ...
    if ((params.get('tab') || 'overview') !== activeTab) {
    ...
    <div className="space-y-6">
      <section className="rounded-xl bg-gradient-to-r from-emerald-700 to-teal-600 p-6 text-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-emerald-100">National command</p>
            <h1 className="mt-2 text-2xl font-bold">Super Fumigation Dashboard</h1>
            <p className="mt-1 max-w-3xl text-sm text-emerald-50">
              Oversee fumigation and cleaning operations across all states and LGAs — manage bookings,
              providers and support escalations from one national console.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-6">
          {loading ? (
            <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
              Loading national fumigation data...
            </div>
          ) : (
            <>
              ...
              <section className="super-fum-bookings-section rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 px-5 py-4">
                  ...
                  <p className="text-xs text-slate-500">Latest service requests across every state and LGA.</p>
                </div>
      ...
      {activeTab === 'operations' && (
        <section className="super-fum-operations-console rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          <div className="mb-4 flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-600 text-white"><FaSprayCan /></div>
            <div>
              <h2 className="font-semibold text-slate-900">National operations console</h2>
              <p className="text-xs text-slate-500">Assign providers, update booking status and run compliance checks nationwide.</p>
            </div>
          </div>
          <FumigationCleaningAdmin
            title="National Fumigation Operations"
            subtitle="Manage fumigation and cleaning bookings across all states and LGAs."
      ...
      {activeTab === 'escalations' && (
        <section className="super-fum-escalations-section rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          <div className="mb-4 flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-600 text-white"><FaClipboardList /></div>
            <div>
              <h2 className="font-semibold text-slate-900">Fumigation support escalations</h2>
              <p className="text-xs text-slate-500">Department tickets escalated from fumigation and cleaning operations.</p>
            </div>
          </div>
...
export default SuperFumigationAdminDashboard;
