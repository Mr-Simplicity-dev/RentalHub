-- Migration 032: Transportation Admin Monitoring System (FIXED VERSION)
-- Add tables for admin, state admin, and super admin monitoring of transportation system
-- FIXED: Removed ambiguous column references in views
...
CREATE TABLE IF NOT EXISTS super_admin_transportation_oversight (
  ...
  oversight_level VARCHAR(20) NOT NULL CHECK (oversight_level IN ('national', 'regional', 'state')),
  region VARCHAR(100),
  state VARCHAR(100),
...
-- 7. Create views for admin monitoring (FIXED VERSION)
...
-- View for super admin transportation oversight (FIXED - removed ambiguous column references)
...
SELECT 
  ...
  COUNT(DISTINCT CASE WHEN sao.oversight_level = 'national' THEN tb.id 
                      WHEN sao.oversight_level = 'regional' AND p.state IN (
                        SELECT s.state_name FROM states s WHERE s.region = sao.region
                      ) THEN tb.id
  ...
  COUNT(DISTINCT CASE WHEN sao.oversight_level = 'national' THEN ts.id 
                      WHEN sao.oversight_level = 'regional' THEN ts.id
  ...
  COUNT(DISTINCT CASE WHEN sao.oversight_level = 'national' THEN saj.state_admin_id 
                      WHEN sao.oversight_level = 'regional' AND saj.state IN (
                        SELECT s.state_name FROM states s WHERE s.region = sao.region
                      ) THEN saj.state_admin_id
...
WHERE u.user_type IN ('super_admin', 'super_financial_admin', 'super_support_admin')
  AND u.deleted_at IS NULL
  AND u.approval_status = 'approved'
GROUP BY sao.super_admin_id, sao.oversight_level, sao.region, sao.state,
