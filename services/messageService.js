...
exports.getConversations = async (req, res) => {
  try {
    ...
    req.logger.error('Get conversations error:', error);
    res.status(500).json({
      success: false,
...
await db.query(
  ...
  [req.user.id, 'Viewed conversation', 'conversation', otherUserId, req.ip]
    ...
    try {
      ...
      req.logger.error('Audit log error:', logError.message);
    }
...
exports.getEligibleRecipients = async (req, res) => {
  try {
    ...
    req.logger.error('Get eligible recipients error:', error);
    res.status(500).json({
      success: false,
...
exports.getEscalations = async (req, res) => {
  try {
    ...
    req.logger.error('Get escalations error:', error);
    res.status(500).json({
      success: false,
...
exports.markEscalationHandled = async (req, res) => {
  try {
    ...
    req.logger.error('Mark escalation handled error:', error);
    return res.status(500).json({
      success: false,
...
exports.convertEscalationToTicket = async (req, res) => {
  try {
    ...
    req.logger.error('Convert escalation ticket error:', error);
    return res.status(500).json({
      success: false,
...
exports.updateEscalationTicketStatus = async (req, res) => {
  try {
    ...
    req.logger.error('Update escalation ticket status error:', error);
    return res.status(500).json({
      success: false,
...
exports.getFlaggedMessages = async (req, res) => {
  try {
    ...
    req.logger.error('Get flagged messages error:', error);
