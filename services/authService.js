...
const validateAndPrepareRegistration = async (payload) => {
  ...
  if (ninVerified || identityType === 'passport') {
    try {
      ...
      req.logger.warn('Non-blocking KYC check failed:', kycError.message);
    }
  }
...
const assignLawyerRoundRobin = async ({ clientUserId, stateId, lgaName }) => {
  ...
  if (lawyersResult.rows.length === 0) {
    req.logger.error(`No available lawyers found for state_id ${stateId} / LGA ${lgaName}`);
    return null;
  }
  ...
  try {
    ...
    req.logger.error('Lawyer assignment notification error:', notifError);
  }
...
const assignAgentRoundRobin = async ({ landlordUserId, stateId, lgaName }) => {
  ...
  if (agentsResult.rows.length === 0) {
    req.logger.error(`No available platform agents found for state_id ${stateId} / LGA ${lgaName}`);
    return null;
  }
  ...
  try {
    ...
    req.logger.error('Agent assignment notification error:', notifError);
  }
  ...
  try {
    ...
    if (preparedRegistration.referralCode) {
      try {
        ...
        req.logger.error('Referral reward credit error (non-fatal):', referralError);
      }
    }
    ...
    if (preparedRegistration.cleanLawyerEmail) {
      try {
        ...
        req.logger.error('Lawyer invite creation error:', inviteError);
      }
    }
    ...
    if (preparedRegistration.user_type === 'landlord' && preparedRegistration.optionalAgentInvite) {
      try {
        ...
        req.logger.error('Agent invite creation error:', agentInviteError);
      }
    }
    ...
    if (preparedRegistration.useRentalhubLawyers) {
      ...
      try {
        ...
        req.logger.error('Lawyer access fee distribution error (non-fatal):', distError);
        }
      }
    ...
    if (preparedRegistration.useRentalhubAgents && tenantRegistrationPayment) {
      ...
      if (preparedRegistration.useRentalhubLawyers) {
        ...
        try {
          ...
          req.logger.error('Error looking up assigned lawyer for agent fee:', lookupError);
        }
      }
      ...
      try {
        ...
        req.logger.error('Round-robin agent assignment error (non-fatal):', roundRobinError);
      }
      ...
      try {
        ...
        req.logger.error('Agent access fee distribution error (non-fatal):', distError);
      }
    }
...
exports.initializeRegistrationPayment = async (req, res) => {
  try {
    ...
    req.logger.error('Registration payment initialization error:', error);
...
exports.getRegistrationFlags = async (req, res) => {
  try {
    ...
    req.logger.error('Registration flags error:', error);
    res.status(500).json({
      success: false,
...
exports.acceptLawyerInvite = async (req, res) => {
  try {
    ...
    try {
      ...
      req.logger.error('Lawyer accepted notification error:', notifError);
    }
    ...
    req.logger.error('Accept lawyer invite error:', error);
...
exports.acceptPlatformLawyerInvite = async (req, res) => {
  try {
    ...
    try {
      ...
      req.logger.error('Platform lawyer accepted notification error:', notifError);
    }
    ...
    req.logger.error('Accept platform lawyer invite error:', error);
...
exports.getLawyerInvites = async (req, res) => {
  try {
    ...
    req.logger.error('Get lawyer invites error:', error);
    return res.status(500).json({
      success: false,
...
exports.resendLawyerInvite = async (req, res) => {
  try {
    ...
    req.logger.error('Resend lawyer invite error:', error);
    return res.status(500).json({
      success: false,
...
exports.updateLawyerInviteEmail = async (req, res) => {
  try {
    ...
    req.logger.error('Update lawyer invite email error:', error);
    return res.status(500).json({
      success: false,
...
exports.checkLawyerPassportForFraud = async (req, res) => {
  try {
    ...
    if (lawyerIdentity.rows.length) {
      ...
      if (!fraudDetected && passportFile.buffer) {
        ...
        try {
          ...
          req.logger.warn('Passport file hash check error (non-fatal):', hashError.message);
        }
      }
      ...
      if (superAdmins.rows.length > 0) {
        try {
          for (const admin of superAdmins.rows) {
            ...
            }).catch(err => req.logger.error('Fraud alert email failed:', err));
          }
          ...
          req.logger.error('Super admin notification failed:', notifyError);
        }
      }
    }
    ...
    req.logger.error('Passport fraud check error:', error);
    return res.status(500).json({
      success: false,
...
exports.uploadPassport = async (req, res) => {
  try {
    ...
    req.logger.error('Passport upload error:', error);
    res.status(500).json({
      success: false,
