...
router.post('/',
  ...
  [
    ...
    body('message_text').trim().notEmpty().customSanitizer(v => v.replace(/<[^>]*>/g, '')).isLength({ max: 50000 }),
    body('property_id').optional().isInt(),
    body('subject').optional().isString().trim().customSanitizer(v => v ? v.replace(/<[^>]*>/g, '') : v).isLength({ min: 1, max: 180 }),
