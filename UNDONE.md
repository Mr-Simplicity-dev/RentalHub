...
- [ ] **Language pack: remaining hardcoded-English files** — 124 files without i18n found. DONE so far: AppLanding, QrCodePage, Terms, Privacy (full), RentSavingsModal (receipt link), WalletFundModal, WalletWithdrawModal, damage/* (Card, Capture, Button — Preview deleted as dead code), PropertyCard, PropertyFilters, FumigationCleaningCatalog, FumigationCleaningAdmin, FumigationCleaningWizard, common/* (ConfirmDialog, InputDialog, SupportReplyActionModal, InternalNotesPanel, ShareMenu, TicketConversationModal). Remaining batches:
  - User-facing components: LivePropertyPhotoCapture, PropertyList, PropertyShareButton, DisputeCreationModal, DisputeQRCode, MapPicker, FumigationCleaningDashboardButton, calls/* (AudioCallPanel, CallNotification, OnlineStatusBadge), common/* (AppealModal, ApprovalTimeline, BackToDashboard, BookingCancelModal, Button, ErrorBoundary, LivePassportCaptureModal, Loader, Modal, RoleBadge, TurnstileWidget, WidgetErrorBoundary)
